...
public class NaturalLanguageProcesser {
	...
	static {
		fillStopWords(IndonesianAnalyzer.getDefaultStopSet().toString(), indonesianStopWords);
		fillStopWords(EnglishAnalyzer.getDefaultStopSet().toString(), englishStopWords);
	}

	// get stop words from a string resulted from Lucene analyser
	private static void fillStopWords(String rawSW, HashSet<String> out) {
