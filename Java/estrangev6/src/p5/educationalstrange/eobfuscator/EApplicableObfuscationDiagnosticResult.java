...
public class EApplicableObfuscationDiagnosticResult {
	// for comment
	private boolean hasSingleLineComment, hasMultiLineComment;
	private boolean hasBlankLinesBeforeStatement;
	private boolean hasPunctuationInSingleLineComment, hasMoreThanNCharsInSingleLineComment;
	private boolean hasPunctuationInMultiLineComment, hasMoreThanNCharsInMultiLineComment;
	...
	private boolean hasStopWordInIdentifier, has_InIdentifier, hasNumberInIdentifier;
	private boolean hasLowercasedCharInIdentifier, hasUppercasedCharInIdentifier;
	private boolean has_ForTransitionInIdentifier, hasCapitalisationForTransitionInIdentifier;
	...
	private boolean hasNonLargestDataTypeForNonFloating, hasNonLargestDataTypeForFloating;
	...
	public static EApplicableObfuscationDiagnosticResult generateDiagnosticResult(ArrayList<ETokenTuple> tokens,
			EObfuscatorSettingTuple set) {
		EApplicableObfuscationDiagnosticResult result = new EApplicableObfuscationDiagnosticResult();

		// for hasBlankLinesBeforeStatement
		HashSet<Integer> commentedLines = new HashSet<>();

		for (int i = 0; i < tokens.size(); i++) {
			ETokenTuple cur = tokens.get(i);
			...
			if (cur.getType().endsWith("COMMENT")) {
				if (cur.getText().startsWith(set.getPrefixSingleLineComment())) {
					...
					text = text.substring(set.getPrefixSingleLineComment().length());
					...
					if (text.trim().length() > set.getnForMaxCharsInSingleLineComment()) {
						result.setHasMoreThanNCharsInSingleLineComment(true);
					}

					// for hasBlankLinesBeforeStatement
					commentedLines.add(cur.getLine());
				} else if (cur.getText().startsWith(set.getPrefixMultiLineComment())) {
					// multi line comment

					// remove the comment prefix and postfix
					text = text.substring(set.getPrefixMultiLineComment().length());
					text = text.substring(0, text.length() - set.getPostfixMultiLineComment().length());
					...
					if (text.trim().length() > set.getnForMaxCharsInMultiLineComment())
						if (set.isFacilitatingMultiLineComment())
				...
				for (int j = 0; j < text.length(); j++) {
					...
					if (c == ' ' && j + 1 < text.length() && text.charAt(j + 1) >= 97 && text.charAt(j + 1) <= 122)
				...
				for (int k = 0; k < commentWords.length; k++) {
					...
					if (set.getHumanLanguage().equals("en")) {
						...
						if (w.equals("plus") || w.equals("minus") || w.equals("times")
								|| (w.equals("divided") && k + 1 < commentWords.length
										&& commentWords[k + 1].equals("by"))
								|| (w.equals("equals") && k + 1 < commentWords.length
										&& commentWords[k + 1].equals("to")))
							result.setHasMathWordInComment(true);

						// for hasSmallNumberWordInComment
						if (w.equals("zero") || w.equals("one") || w.equals("two") || w.equals("three")
								|| w.equals("four") || w.equals("five") || w.equals("six") || w.equals("seven")
								|| w.equals("eight") || w.equals("nine") || w.equals("ten") || w.equals("eleven")
								|| w.equals("twelve"))
						...
						// for hasMathWordInComment(
						if (w.equals("tambah") || w.equals("kurang") || w.equals("kali")
								|| (w.equals("dibagi") && k + 1 < commentWords.length
										&& commentWords[k + 1].equals("dengan"))
								|| (w.equals("sama") && k + 1 < commentWords.length
										&& commentWords[k + 1].equals("dengan")))
							result.setHasMathWordInComment(true);

						// for hasSmallNumberWordInComment
						if (w.equals("nol") || w.equals("satu") || w.equals("dua") || w.equals("tiga")
								|| w.equals("empat") || w.equals("lima") || w.equals("enam") || w.equals("tujuh")
								|| w.equals("delapan") || w.equals("sembilan") || w.equals("sepuluh")
								|| w.equals("sebelas") || (w.equals("dua") && k + 1 < commentWords.length
										&& commentWords[k + 1].equals("belas")))
							result.setHasSmallNumberWordInComment(true);
					}

					// for hasSmallNumberWordInComment
					if (w.equals("0") || w.equals("1") || w.equals("2") || w.equals("3") || w.equals("4")
							|| w.equals("5") || w.equals("6") || w.equals("7") || w.equals("8") || w.equals("9")
							|| w.equals("10") || w.equals("11") || w.equals("12"))
				...
				if (lastIndex != -1 && lastIndex + System.lineSeparator().length() < text.length())
				...
				ArrayList<String> identWords = ECodeObfuscatorIdentifier.tokenizeIdentifier(text);
				for (String w : identWords) {
					// for hasStopWordInIdentifier
					if (NaturalLanguageProcesser.isStopWord(w.toLowerCase(), set.getHumanLanguage())) {
				...
				for (int j = 0; j < text.length(); j++) {
					...
					if (c >= 65 && c <= 90) {
						 ...
						 * mark hasCapitalisationForTransitionInIdentifier
						 */
						if (j != 0 && text.charAt(j - 1) >= 97 && text.charAt(j - 1) <= 122)
					...
					/*
					 * if at least a vocal is located in the middle of identifier, mark
					 * hasVocalsToBeRemovedForAcronymInIdentifiers
					 */
					if ((c == 'a' || c == 'A' || c == 'i' || c == 'I' || c == 'u' || c == 'U' || c == 'e' || c == 'E'
							|| c == 'o' || c == 'O') && j != 0 && j != text.length() - 1)
						result.setHasVocalsToBeRemovedForAcronymInIdentifiers(true);
				}
			} else if (set.isDataTypeSensitive() && (cur.getType().equals("'byte'") || cur.getType().equals("'short'")
					|| cur.getType().equals("'int'"))) {
				// for hasNonLargestDataTypeForNonFloating
				result.setHasNonLargestDataTypeForNonFloating(true);

			} else if (set.isDataTypeSensitive() && cur.getType().equals("'float'")) {
				// for hasNonLargestDataTypeForFloating
				result.setHasNonLargestDataTypeForFloating(true);

			} else if ((cur.getText().startsWith("\"")
					|| (set.isSingleQuoteAlsoStringSeparator() && cur.getText().startsWith("'")))
					&& text.length() >= 2
					) {
				// for string literal: the length should be at least 2 for processing
				...
				for (int k = 0; k < StringLiteralWords.length; k++) {
					...
					if (set.getHumanLanguage().equals("en")) {
						...
						if (w.equals("plus") || w.equals("minus") || w.equals("times")
								|| (w.equals("divided") && k + 1 < StringLiteralWords.length
										&& StringLiteralWords[k + 1].equals("by"))
								|| (w.equals("equals") && k + 1 < StringLiteralWords.length
										&& StringLiteralWords[k + 1].equals("to")))
						...
						// for hasMathWordInString
						if (w.equals("tambah") || w.equals("kurang") || w.equals("kali")
								|| (w.equals("dibagi") && k + 1 < StringLiteralWords.length
										&& StringLiteralWords[k + 1].equals("dengan"))
								|| (w.equals("sama") && k + 1 < StringLiteralWords.length
										&& StringLiteralWords[k + 1].equals("dengan")))
							result.setHasMathWordInString(true);
						...
						if (w.equals("nol") || w.equals("satu") || w.equals("dua") || w.equals("tiga")
								|| w.equals("empat") || w.equals("lima") || w.equals("enam") || w.equals("tujuh")
								|| w.equals("delapan") || w.equals("sembilan") || w.equals("sepuluh")
								|| w.equals("sebelas") || (w.equals("dua") && k + 1 < StringLiteralWords.length
										&& StringLiteralWords[k + 1].equals("belas")))
							result.setHasSmallNumberWordInString(true);
					}
			...
			if (cur.getText().matches("[a-zA-Z0-9]+")) {
				// for hasBlankLinesBeforeStatement
				ETokenTuple prevSyntaxT = null;
				for (int j = i - 1; j >= 0; j--) {
					ETokenTuple temp = tokens.get(j);
					if (!temp.getType().endsWith("COMMENT") && !temp.getType().equals("WS")) {
						prevSyntaxT = temp;
						break;
					}
				}
				if (prevSyntaxT != null && cur.getLine() - prevSyntaxT.getLine() >= 2
	...
	public String toString() {
		...
		out += "hasBlankLinesBeforeStatement:        " + hasBlankLinesBeforeStatement + "\n";
		out += "hasPunctuationInSingleLineComment:   " + hasPunctuationInSingleLineComment + "\n";
		out += "hasMoreThanNCharsInSingleLineComment:" + hasMoreThanNCharsInSingleLineComment + "\n";
		out += "[j]hasPunctuationInMultiComment:     " + hasPunctuationInMultiLineComment + "\n";
		out += "[j]hasMoreThanNCharsInMultiComment:  " + hasMoreThanNCharsInMultiLineComment + "\n";
		out += "[j]hasNewlineInMultiLineComment:     " + hasNewlineInMultiLineComment + "\n";
		out += "hasLowercasedFirstWordCharInComment: " + hasLowercasedFirstWordCharInComment + "\n";
		out += "hasLowercasedCharsInComment:         " + hasLowercasedCharsInComment + "\n";
		out += "hasUppercasedCharsInComment:         " + hasUppercasedCharsInComment + "\n";
		out += "hasConjunctionSymbolInComment:       " + hasConjunctionSymbolInComment + "\n";
		out += "hasConjunctionWordInComment:         " + hasConjunctionWordInComment + "\n";
		out += "hasMathSymbolInComment:              " + hasMathSymbolInComment + "\n";
		out += "hasMathWordInComment:                " + hasMathWordInComment + "\n";
		out += "hasSmallNumberInComment:             " + hasSmallNumberInComment + "\n";
		out += "hasSmallNumberWordInComment:         " + hasSmallNumberWordInComment + "\n";

		out += "\nwhitespace:\n";
		out += "hasBlankLines:                       " + hasBlankLines + "\n";
		out += "[j]hasTabsOrSpacesBeforeStatement:   " + hasTabsOrSpacesBeforeStatement + "\n";
		...
		out += "hasStopWordInIdentifier:             " + hasStopWordInIdentifier + "\n";
		out += "has_InIdentifier:                    " + has_InIdentifier + "\n";
		out += "hasNumberInIdentifier:               " + hasNumberInIdentifier + "\n";
		out += "hasLowercasedCharInIdentifier:       " + hasLowercasedCharInIdentifier + "\n";
		out += "hasUppercasedCharInIdentifier:       " + hasUppercasedCharInIdentifier + "\n";
		out += "has_ForTransitionInIdentifier:       " + has_ForTransitionInIdentifier + "\n";
		out += "hasCapitalForTransitionInIdentifier: " + hasCapitalisationForTransitionInIdentifier + "\n";
		out += "hasIdentifiersWithLengthMoreThan1:   " + hasIdentifiersWithLengthMoreThan1 + "\n";
		out += "hasVocalsForAcronymInIdentifiers:    " + hasVocalsToBeRemovedForAcronymInIdentifiers + "\n";

		out += "\n[j]hasNonLargestDataTypeForNonFloat: " + hasNonLargestDataTypeForNonFloating + "\n";
		out += "[j]hasNonLargestDataTypeForFloat:    " + hasNonLargestDataTypeForFloating + "\n";
		out += "hasStringLiteral:                    " + hasStringLiteral + "\n";
		out += "hasLowercasedFirstWordCharInString:  " + hasLowercasedFirstWordCharInString + "\n";
		out += "hasLowercasedCharsInString:          " + hasLowercasedCharsInString + "\n";
		out += "hasUppercasedCharsInString:          " + hasUppercasedCharsInString + "\n";
		out += "hasConjunctionSymbolInString:        " + hasConjunctionSymbolInString + "\n";
		out += "hasConjunctionWordInString:          " + hasConjunctionWordInString + "\n";
		...
		out += "hasSmallNumberWordInString:          " + hasSmallNumberWordInString + "\n";
		out += "hasFloatingNumber:                   " + hasFloatingNumber + "\n";

		return out;
	}

	public static boolean[] getApplicableDisguises(EApplicableObfuscationDiagnosticResult d,
			EObfuscatorSettingTuple set) {
		/*
		 * this method returns an array of booleans in which each element represents
		 * whether a particular disguise is applicable. All disguises are applicable
		 * except whitespace reformatting; it is excluded for simplicity as the process
		 * requires multiple file accesses from an external library. We also exclude C0,
		 * C2, and C4 as if the removal probability is low and/or you are unlucky, no
		 * comments are removed, providing no disguises.
		 */
...
//		if ((d.isHasSingleLineComment() || d.isHasMultiLineComment()) && set.isFacilitatingMultiLineComment())
//			result[4] = true;
		...
		// content is randomly generated.
		if (d.isHasBlankLinesBeforeStatement() && set.isFacilitatingMultiLineComment())
			result[7] = true;
		...
		if (d.isHasPunctuationInSingleLineComment() && set.isFacilitatingMultiLineComment())
			result[9] = true;

		// 10 changing each single-line comment to the multi-line one with n
		// characters per line.
		if (d.isHasMoreThanNCharsInSingleLineComment() && set.isFacilitatingMultiLineComment())
			result[10] = true;
		...
		// newlines removed.
		if (d.isHasNewlineInMultiLineComment() && set.isFacilitatingMultiLineComment())
			result[13] = true;
		...
		if (d.isHasPunctuationInMultiLineComment() && set.isFacilitatingMultiLineComment())
			result[15] = true;

		// 16 splitting each multi-line comment to several single-line comments
		// with n characters per line.
		if (d.isHasMoreThanNCharsInMultiLineComment() && set.isFacilitatingMultiLineComment())
			result[16] = true;
		...
		// for Python.
		if (d.isHasTabsOrSpacesBeforeStatement() && set.isWhitespacesIgnorable())
			result[28] = true;
...
//			result[33] = true;
		...
		// 34 reformat the whitespaces. SKIPPED FOR EFFICIENCY.
		result[34] = false;
		...
		if (d.isHasNonLargestDataTypeForNonFloating() && set.isDataTypeSensitive())
			result[45] = true;
	...
	public void setHasBlankLinesBeforeStatement(boolean hasBlankLinesBeforeStatement) {
	...
	public void setHasPunctuationInSingleLineComment(boolean hasPunctuationInSingleLineComment) {
	...
	public void setHasMoreThanNCharsInSingleLineComment(boolean hasMoreThanNCharsInSingleLineComment) {
	...
	public void setHasPunctuationInMultiLineComment(boolean hasPunctuationInMultiLineComment) {
	...
	public void setHasMoreThanNCharsInMultiLineComment(boolean hasMoreThanNCharsInMultiLineComment) {
	...
	public void setHasNewlineInMultiLineComment(boolean hasNewlineInMultiLineComment) {
	...
	public void setHasLowercasedFirstWordCharInComment(boolean hasLowercasedFirstWordCharInComment) {
	...
	public void setHasLowercasedCharsInComment(boolean hasLowercasedCharsInComment) {
	...
	public void setHasUppercasedCharsInComment(boolean hasUppercasedCharsInComment) {
	...
	public void setHasConjunctionSymbolInComment(boolean hasConjunctionSymbolInComment) {
	...
	public void setHasConjunctionWordInComment(boolean hasConjunctionWordInComment) {
	...
	public void setHasSmallNumberWordInComment(boolean hasSmallNumberWordInComment) {
	...
	public void setHasTabsOrSpacesBeforeStatement(boolean hasTabsOrSpacesBeforeStatement) {
	...
	public void setHasLowercasedCharInIdentifier(boolean hasLowercasedCharInIdentifier) {
	...
	public void setHasUppercasedCharInIdentifier(boolean hasUppercasedCharInIdentifier) {
	...
	public void setHas_ForTransitionInIdentifier(boolean has_ForTransitionInIdentifier) {
	...
	public void setHasCapitalisationForTransitionInIdentifier(boolean hasCapitalisationForTransitionInIdentifier) {
	...
	public void setHasIdentifiersWithLengthMoreThan1(boolean hasIdentifiersWithLengthMoreThan1) {
	...
	public void setHasVocalsToBeRemovedForAcronymInIdentifiers(boolean hasVocalsToBeRemovedForAcronymInIdentifiers) {
	...
	public void setHasNonLargestDataTypeForNonFloating(boolean hasNonLargestDataTypeForNonFloating) {
	...
	public void setHasNonLargestDataTypeForFloating(boolean hasNonLargestDataTypeForFloating) {
	...
	public void setHasLowercasedFirstWordCharInString(boolean hasLowercasedFirstWordCharInString) {
	...
	public void setHasConjunctionSymbolInString(boolean hasConjunctionSymbolInString) {
