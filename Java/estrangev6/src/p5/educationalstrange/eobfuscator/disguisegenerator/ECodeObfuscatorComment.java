...
public class ECodeObfuscatorComment {
	...
	public static void c01JavaRemovingSomeSingleLineComments(ArrayList<ETokenTuple> tokenString, double probability) {
		/*
	...
	public static void c01PythonRemovingSomeSingleLineComments(ArrayList<ETokenTuple> tokenString, double probability) {
		/*
	...
	private static void _commentRemovingSomeComments(ArrayList<ETokenTuple> tokenString, double probability,
			String prefix) {
		/*
				...
				if (t.getText().startsWith(prefix)) {
					...
					if (guess <= probability) {
						...
						if (i > 0 && i < tokenString.size()) {
							ETokenTuple prev = tokenString.get(i - 1);
							t = tokenString.get(i);
						...
						for (; j < tokenString.size(); j++) {
							ETokenTuple temp = tokenString.get(j);
	...
	public static void c02JavaRemovingAllSingleLineComments(ArrayList<ETokenTuple> tokenString) {
		// remove all single line comments from java token string
		c01JavaRemovingSomeSingleLineComments(tokenString, 1);
	}

	public static void c02PythonRemovingAllSingleLineComments(ArrayList<ETokenTuple> tokenString) {
		// remove all single line comments from python token string
		c01PythonRemovingSomeSingleLineComments(tokenString, 1);
	}

	public static void c03JavaRemovingSomeMultiLineComments(ArrayList<ETokenTuple> tokenString, double probability) {
		/*
	...
	public static void c04JavaRemovingAllMultiLineComments(ArrayList<ETokenTuple> tokenString) {
		// remove all multi line comments from java token string
		c03JavaRemovingSomeMultiLineComments(tokenString, 1);
	}

	public static void c05JavaRemovingSomeComments(ArrayList<ETokenTuple> tokenString, double probability) {
	...
	public static void c06JavaRemovingAllComments(ArrayList<ETokenTuple> tokenString) {
		// remove all comments
		c02JavaRemovingAllSingleLineComments(tokenString);
		c04JavaRemovingAllMultiLineComments(tokenString);
	}

	public static void c07JavaAddingSingleLineComment(ArrayList<ETokenTuple> tokenString, String language) {
		// quite similar with the called method except it deals with single-line
		// comments.
		_commentAddingComment(tokenString, "//", "", language);
	}

	public static void c07PythonAddingSingleLineComment(ArrayList<ETokenTuple> tokenString, String language) {
		// quite similar with the called method except it deals with single-line
		// comments.
		_commentAddingComment(tokenString, "#", "", language);
	}

	private static void _commentAddingComment(ArrayList<ETokenTuple> tokenString, String prefix, String postfix,
			String language) {
		/*
			...
			if (t.getType().endsWith("COMMENT")) {
				...
				if (t.getLine() - prevSyntaxT.getLine() >= 2) {
					...
					if (!commentedLines.contains(t.getLine() - 1)) {
						int randompos = r.nextInt(addedCommentAlternatives.length);
						// embed the new comment
						tokenString.add(i, new ETokenTuple(prefix + " " + addedCommentAlternatives[randompos] + " "
								+ postfix + System.lineSeparator(), "LINE_COMMENT", t.getLine()));
						...
						if (i - 2 >= 0 && tokenString.get(i - 2).getType().equals("WS")) {
							ETokenTuple prevWhitespace = tokenString.get(i - 2);
							// create a copied whitespace token
							ETokenTuple ntok = new ETokenTuple(
	...
	public static void c08JavaAddingMultiLineComment(ArrayList<ETokenTuple> tokenString, String language) {
	...
	public static void c09JavaReplacingSingleToMultiComments(ArrayList<ETokenTuple> tokenString) {
	...
	public static void c11JavaReplacingSingleToMultiCommentsWithNCharsPerLine(ArrayList<ETokenTuple> tokenString,
			int n) {
		/*
	...
	private static void _commentReplacingCommentToSeveralSingleLineCommentsWithSomePunctuationsAsLineDelimiters(
			ArrayList<ETokenTuple> tokenString, String oldprefix, String oldpostfix, String singleLinePrefix) {
		/*
					...
					if (text.contains(".") || text.contains("?") || text.contains("!") || text.contains(";")) {
						...
						for (int k = 0; k < text.length(); k++) {
							...
							if (c == '.' || c == '?' || c == '!' || c == ';') {
								...
								tokenString.add(i, new ETokenTuple(singleLinePrefix + " " + lineText.trim(),
										t.getType(), t.getLine() + addedLineCounter));
								i++;
								tokenString.add(i,
										new ETokenTuple(System.lineSeparator(), "WS", t.getLine() + addedLineCounter));
								i++;
								// clean the line text
								lineText = "";
								// add line counter
						...
						if (lineText.trim().length() > 0) {
							// add a new comment
							tokenString.add(i, new ETokenTuple(singleLinePrefix + " " + lineText.trim(), t.getType(),
	...
	public static void c13JavaReplacingSingleToNSingleLineCommentsWithNCharsPerLine(ArrayList<ETokenTuple> tokenString,
			int n) {
		_commentReplacingCommentToSeveralSingleLineCommentsWithNCharsPerLine(tokenString, n, "//", "", "//");
	}

	public static void c13PythonReplacingSingleToNSingleCommentsWithNCharsPerLine(ArrayList<ETokenTuple> tokenString,
			int n) {
		_commentReplacingCommentToSeveralSingleLineCommentsWithNCharsPerLine(tokenString, n, "#", "", "#");
	}

	private static void _commentReplacingCommentToSeveralSingleLineCommentsWithNCharsPerLine(
			ArrayList<ETokenTuple> tokenString, int n, String oldprefix, String oldpostfix, String newPrefix) {
		/*
						...
						for (String w : words) {
							if (lineText.length() + w.length() + 1 <= n) {
								...
								if (lineText.trim().length() > 0) {
									...
									tokenString.add(i, new ETokenTuple(newPrefix + " " + lineText.trim(), t.getType(),
											t.getLine() + addedLineCounter));
									i++;
									// add a newline
									tokenString.add(i, new ETokenTuple(System.lineSeparator(), "WS",
											t.getLine() + addedLineCounter));
									i++;
									// add the line counter
									addedLineCounter++;
	...
	public static void c14JavaReplacingMultiToSingleCommentsWithNewlinesRemoved(ArrayList<ETokenTuple> tokenString) {
		String oldprefix = "/*";
		String oldpostfix = "*/";
		String newprefix = "//";
		String newpostfix = "";
	...
	public static void c15JavaSplitMultiToNSingleCommentsWithNewlineAsSeparator(ArrayList<ETokenTuple> tokenString) {
		String oldprefix = "/*";
		String oldpostfix = "*/";
		String newprefix = "//";
		String newpostfix = "";
			...
			if (t.getType().endsWith("COMMENT")) {
				if (t.getText().startsWith(oldprefix)) {
					...
					for (int k = 0; k < lines.size(); k++) {
						...
						ETokenTuple temp = new ETokenTuple(l, t.getType(), t.getLine() + k);
						tokenString.add(i + 2 * k, temp);
						if (k != lines.size() - 1) {
							// if not the last, add newline token
							// the last one will use the existing one
							ETokenTuple ws = new ETokenTuple(System.lineSeparator(), "WS", t.getLine() + k);
	...
	public static void c17JavaReplacingMultiToSeveralSingleCommentsWithNCharsPerLine(ArrayList<ETokenTuple> tokenString,
			int n) {
		_commentReplacingCommentToSeveralSingleLineCommentsWithNCharsPerLine(tokenString, n, "/*", "*/", "//");
	}

	public static void c18JavaCapitalisingFirstCharEachWord(ArrayList<ETokenTuple> tokenString) {
		/*
	...
	public static void c18PythonCapitalisingFirstCharEachWord(ArrayList<ETokenTuple> tokenString) {
		/*
		 * for each comment, split based on whitespace and uppercase the first char of
		 * each word.
		 */
	...
	private static void _capitalisingFirstCharEachWord(ArrayList<ETokenTuple> tokenString, String commentSinglePrefix,
	...
	public static void c19CapitalisingAllChars(ArrayList<ETokenTuple> tokenString) {
	...
	public static void c20DecapitalisingAllChars(ArrayList<ETokenTuple> tokenString) {
	...
	public static void c21JavaReplacingConjuctionSymbolsWithWords(ArrayList<ETokenTuple> tokenString, String language) {
		_replacingConjuctionSymbolsWithWords(tokenString, language, "//", "/*", "*/");
	}

	public static void c21PythonReplacingConjuctionSymbolsWithWords(ArrayList<ETokenTuple> tokenString,
			String language) {
		_replacingConjuctionSymbolsWithWords(tokenString, language, "#", "", "");
	}

	private static void _replacingConjuctionSymbolsWithWords(ArrayList<ETokenTuple> tokenString, String language,
	...
	public static void c22JavaReplacingConjuctionWordsWithSymbols(ArrayList<ETokenTuple> tokenString, String language) {
		// replace all conjunction symbol with word
		_replacingConjuctionWordsWithSymbols(tokenString, language, "//", "/*", "*/");
	}

	public static void c22PythonReplacingConjuctionWordsWithSymbols(ArrayList<ETokenTuple> tokenString,
			String language) {
		// replace all conjunction symbol with word
		_replacingConjuctionWordsWithSymbols(tokenString, language, "#", "", "");
	}

	private static void _replacingConjuctionWordsWithSymbols(ArrayList<ETokenTuple> tokenString, String language,
			String commentSinglePrefix, String commentMultiPrefix, String commentMultiPostfix) {
		// replace all conjunction symbol with word
	...
	public static void c23JavaReplacingMathSymbolsWithWords(ArrayList<ETokenTuple> tokenString, String language) {
		// replace all math symbol with word
		_replacingMathSymbolsWithWords(tokenString, language, "//", "/*", "*/");
	}

	public static void c23PythonReplacingMathSymbolsWithWords(ArrayList<ETokenTuple> tokenString, String language) {
		// replace all math symbol with word
		_replacingMathSymbolsWithWords(tokenString, language, "#", "", "");
	}

	private static void _replacingMathSymbolsWithWords(ArrayList<ETokenTuple> tokenString, String language,
			String commentSinglePrefix, String commentMultiPrefix, String commentMultiPostfix) {
		// replace all math symbol with word
	...
	public static void c24JavaReplacingMathWordsWithSymbols(ArrayList<ETokenTuple> tokenString, String language) {
		// replace all math symbol with word
		_replacingMathWordsWithSymbols(tokenString, language, "//", "/*", "*/");
	}

	public static void c24PythonReplacingMathWordsWithSymbols(ArrayList<ETokenTuple> tokenString, String language) {
		// replace all math symbol with word
		_replacingMathWordsWithSymbols(tokenString, language, "#", "", "");
	}

	private static void _replacingMathWordsWithSymbols(ArrayList<ETokenTuple> tokenString, String language,
			String commentSinglePrefix, String commentMultiPrefix, String commentMultiPostfix) {
		// replace all math symbol with word
	...
	public static void c25JavaReplacingSmallNumbersWithWords(ArrayList<ETokenTuple> tokenString, String language) {
		// replace all small numbers with word
		_replacingSmallNumbersWithWords(tokenString, language, "//", "/*", "*/");
	}

	public static void c25PythonReplacingSmallNumbersWithWords(ArrayList<ETokenTuple> tokenString, String language) {
		// replace all small numbers with word
		_replacingSmallNumbersWithWords(tokenString, language, "#", "", "");
	}

	private static void _replacingSmallNumbersWithWords(ArrayList<ETokenTuple> tokenString, String language,
			String commentSinglePrefix, String commentMultiPrefix, String commentMultiPostfix) {
		// replace all small numbers with word
	...
	public static void c26JavaReplacingSmallNumberWordsWithNumbers(ArrayList<ETokenTuple> tokenString,
			String language) {
		// replace all small number words with numbers
		_replacingSmallNumberWordsWithNumbers(tokenString, language, "//", "/*", "*/");
	}

	public static void c26PythonReplacingSmallNumberWordsWithNumbers(ArrayList<ETokenTuple> tokenString,
			String language) {
		// replace all small number words with numbers
		_replacingSmallNumberWordsWithNumbers(tokenString, language, "#", "", "");
	}

	private static void _replacingSmallNumberWordsWithNumbers(ArrayList<ETokenTuple> tokenString, String language,
			String commentSinglePrefix, String commentMultiPrefix, String commentMultiPostfix) {
		// replace all small number words with numbers
	...
	public static void c27JavaAnonymisingCommentContents(ArrayList<ETokenTuple> tokenString, String language) {
	...
	public static void c27PythonAnonymisingCommentContents(ArrayList<ETokenTuple> tokenString, String language) {
