	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if(isset($_POST['id']) == true){
			...
			if ($db->query($sql) === TRUE) {
			  ...
			  echo "<script>alert('The lecturer cannot be deleted since they have created at least one course.');</script>";
			}
		}
	}
?>
<html>
	...
	<meta name="viewport" content="width=device-width, initial-scale=1">
		<title> E-STRANGE: Admin home</title>
...
<script type="text/javascript" src="datatables/responsive.bootstrap5.min.js"></script>
  ...
  <body>
		...
		<?php setHeaderAdmin("dashboard", "Admin home"); ?>
		<div class="container my-3">
    <div class="bodycontent">
		<div class="row d-flex justify-content-center align-items-center">
				<div class="col-md-6">
					<div class="infotitle fs-1">Lecturer list:</div>
				</div>
				<div class="col-md-6 buttontambah">
					<button class="btn btn-primary addcourse" onclick="window.open('admin_lecturer_add_bulk.php', '_self');">Bulk add</button>
            		<button class="btn btn-primary addcourse" onclick="window.open('admin_lecturer_add.php', '_self');">Manual add</button>
				</div>
			</div>
	
        <div class="tablecontainer">
            <table id="lecturerTable" class="table table-bordered table-striped responsive nowrap" style="width:100%" >
                <thead>
                    <tr>
                        <th >Username</th>
                        <th >Name</th>
                ...
                <tbody>
                    <?php
                        // Your PHP code
                        $sql = "SELECT user_id, username, name, email FROM user WHERE role = 'lecturer'";
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
								...
								<td>
									<div class=\"d-flex\">
										<form class=\" me-2\" action=\"admin_lecturer_update.php\" method=\"post\">
											<input type=\"hidden\" name=\"id\" value=\"".$row['user_id']."\">
											<button class=\"btn btn-warning\" type=\"submit\">Update</button>
										</form>
										<div class=\" me-2\" action=\"". htmlentities($_SERVER['PHP_SELF']). "\" method=\"post\">
            ...
            <!-- <button class="btn btn-outline-primary addcourse" onclick="window.open('admin_lecturer_add_bulk.php', '_self');">Bulk add</button>
            <button class="btn btn-outline-primary addcourse" onclick="window.open('admin_lecturer_add.php', '_self');">Manual add</button> -->
        </div>
    </div>
</div>
