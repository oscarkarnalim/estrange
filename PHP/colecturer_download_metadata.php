...
// header
$csvrow = array('Username', 'Name', 'Attempt','Submission time', 'Late submission?','Description');
...
while($row  = $resultt->fetch_assoc()){
	...
	if($row['submission_time'] > $row['submission_close_time'])
		$isLate = "LATE";
	$csvrow = array($row['username'], $row['name'], $row['attempt'],$row['submission_time'], $isLate, $row['description']);
	// generate csv rows from the inner arrays
	fputcsv($f, $csvrow, $delimiter); 
}
