  ...
  <body>
		<?php
			if($_SESSION['role'] == 'lecturer')
				setHeaderLecturer("colecturer courses", "Co-lecturer courses");
			else 
				setHeaderStudent("colecturer courses", "Co-lecturer courses");
		?>
		<div class="container my-3">
			<div class="bodycontent">
				<div class="infotitle fs-1"> Co-lecturer course list: </div>
				<table id="colecturerDashboardTable" class="table table-bordered table-striped responsive nowrap" style="width:100%" >
					<thead>
					<tr>
						<th>Name</th>
						<th>Description</th>
				...
				<tbody>
							...
							<?php
								...
								WHERE colecturer.user_id = '".$_SESSION['user_id']."' ORDER BY time_created DESC";
								...
								if ($result->num_rows > 0) {
									while($row = $result->fetch_assoc()) {
										...
										echo "		<td class=\"tdactions\">
														<form class=\"invisform\" action=\"colecturer_assessments.php\" method=\"post\">
															<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
															<input type=\"hidden\" name=\"name\" value=\"".$row['name']."\">
															<button class=\"btn btn-primary w-100\" type=\"submit\">assessments</button>
														</form>
														<form class=\"invisform\" action=\"colecturer_students.php\" method=\"post\">
															<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
															<input type=\"hidden\" name=\"name\" value=\"".$row['name']."\">
															<button class=\"btn btn-primary w-100\" type=\"submit\">students</button>
														</form>";
										if($row['is_active'] == 1)
											echo "<form class=\"invisform\" action=\"colecturer_game.php\" method=\"get\">
												<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
												<button class=\"btn btn-primary w-100\" type=\"submit\">game</button>
											</form>";
										echo "												<form class=\"invisform\" action=\"colecturer_course_update.php\" method=\"post\">
																<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
			var table = new DataTable('#colecturerDashboardTable', {
				responsive: true,
				pageLength: 3,
