	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if(isset($_POST['cname']) == true){
		   // data sent from form
			 ...
			 $myisactive = mysqli_real_escape_string($db,$_POST['is_active']);
		   ...
		   if($errorMessage == ""){
		       ...
		       $sql = "UPDATE assessment SET name = '".$myname."', description = '".$mydesc."',
						...
						course_id = '".$mycourseid."', submission_file_extension = '".$myacceptedfileext."', allow_late_submission = '".$myallowlatesubmission."',is_active = '".$myisactive."'";
...
<html>
  ...
  <body onload="initialise();" >
		<?php
		  setHeaderLecturer("courses", "Update assessments");
		?>
		...
		<?php
			// get the old values
			$sql = "SELECT name, description, submission_open_time, submission_close_time, submission_file_extension, allow_late_submission, is_active
