...
<html>
	...
	<meta name="viewport" content="width=device-width, initial-scale=1">
		<title> E-STRANGE: Admin Course</title>
...
<script type="text/javascript" src="datatables/responsive.bootstrap5.min.js"></script>
  ...
  <body>
		<?php
		  setHeaderAdmin("courses", "Admin Course");
		?>
		...
		<div class="bodycontent">
			<div class="row d-flex justify-content-center align-items-center">
				...
				<div class="col-md-6 buttontambah">
					<button class="btn btn-primary addcourse"  onclick="window.open('admin_course_add.php', '_self');">Add course</button>
				</div>
			</div>
			<div class="tablecontainer">
            <table id="adminDashboardTable" class="table table-bordered table-striped responsive nowrap" style="width:100%" >
				<thead>
					<tr>
						<th >Name</th>
						<th>Description</th>
				...
				<tbody>
						<?php
							// get all courses for a particular admin
							...
							if ($result->num_rows > 0) {
							    while($row = $result->fetch_assoc()) {
									...
									echo "		<td class=\"tdactions\">
														<form class=\"invisform\" action=\"admin_assessments.php\" method=\"post\">
															<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
															<input type=\"hidden\" name=\"name\" value=\"".$row['name']."\">
															<button class=\"btn btn-primary w-100\" type=\"submit\">assessments</button>
														</form>
														<form class=\"invisform\" action=\"admin_students.php\" method=\"post\">
															<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
															<input type=\"hidden\" name=\"name\" value=\"".$row['name']."\">
															<button class=\"btn btn-primary w-100\" type=\"submit\">Students</button>
														</form>
														<form class=\"invisform\" action=\"admin_colecturer.php\" method=\"post\">
															<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
															<input type=\"hidden\" name=\"name\" value=\"".$row['name']."\">
									...
									if($row['is_active'] == 1){
										// if game is active
										echo				"<form class=\"invisform\" action=\"admin_game.php\" method=\"get\">
																<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
																<button class=\"btn btn-primary w-100\" type=\"submit\">Game</button>
															</form>";
									}
									echo												"<form class=\"invisform\" action=\"admin_course_update.php\" method=\"post\">
															<input type=\"hidden\" name=\"id\" value=\"".$row['course_id']."\">
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
			var table = new DataTable('#adminDashboardTable', {
				responsive: true,
				pageLength: 3,
